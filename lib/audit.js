...
function getAuditLog(landingId) {
  ensureAuditDir();
...
function writeAuditLog(landingId, entries) {
  ensureAuditDir();
  const filePath = getAuditFilePath(landingId);
 ...
 */
function logAudit(landingId, { action, actor, isAdmin, details, metadata, versionIds }) {
  const entries = getAuditLog(landingId);
  ...
  writeAuditLog(landingId, trimmedEntries);
 ...
 */
function getAuditLogPaginated(landingId, { limit = 50, offset = 0 } = {}) {
  const entries = getAuditLog(landingId);
 ...
 * @param {string} landingId - The landing ID
 */
function deleteAuditLog(landingId) {
  const filePath = getAuditFilePath(landingId);
  if (fs.existsSync(filePath)) {

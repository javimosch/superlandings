...
async function createVersion(landing, description = '', auditId = null) {
  ...
  if (getEngine() === 'mongo') {
    ...
    await col.updateOne(
      ...
      { $set: { ...metadata, content: htmlContent, _id: `${landing.id}:${versionId}` } },
      { upsert: true }
    );
  } else {
    fs.writeFileSync(
...
async function rollbackToVersion(landing, versionId) {
  ...
  if (!fs.existsSync(zipPath)) {
    if (getEngine() === 'mongo' && landing.type === 'html') {
...
async function deleteVersion(landingId, versionId) {
  ...
  if (fs.existsSync(versionDir)) {
    ...
    if (getEngine() === 'mongo') {
      const col = await getCollection('versions');
      await col.deleteOne({ _id: `${landingId}:${versionId}` });
    }
    return true;
  }
...
function getLandingFsDir(landing) {
  if (getEngine() === 'mongo' && landing && landing.type === 'html') {

...
async function migrateExistingLandings() {
  ...
  for (const landing of landings) {
    ...
    const existingVersions = await getVersions(landing.id);
    ...
    if (existingVersions.length > 0) {
      ...
      try {
        const initialVersion = await createVersion(landing, 'Initial version - migration');

...
const { readDB: readJsonDB, writeDB: writeJsonDB, DATA_DIR, readDirectoryFilesSync } = require('./db');
const { LANDINGS_DIR } = require('./db');
const { safeSlugPath } = require('./utils');
...
async function syncLandingsToMongo(landings, { forceSync } = {}) {
  ...
  const enriched = (landings || []).map(l => {
    if (l && l.type === 'html' && typeof l.content !== 'string') {
      try {
        const indexPath = path.join(safeSlugPath(LANDINGS_DIR, l.slug), 'index.html');
        if (fs.existsSync(indexPath)) {
          return { ...l, content: fs.readFileSync(indexPath, 'utf-8') };
        }
      } catch (e) {
        console.warn(`[store] Could not read index.html for slug ${l.slug}: ${e.message}`);
      }
    }
    if (l && l.type === 'virtual' && (!l.files || l.files.length === 0)) {
      try {
        const landingDir = safeSlugPath(LANDINGS_DIR, l.slug);
        if (fs.existsSync(landingDir)) {
          const files = [];
          readDirectoryFilesSync(landingDir, '', files);
          return { ...l, files };
        }
      } catch (e) {
        console.warn(`[store] Could not read virtual files for slug ${l.slug}: ${e.message}`);
      }
    }
    return l;
  });

  const ops = enriched.map(l => ({
    updateOne: {
      filter: { _id: l.id },
      update: { $set: { ...l, _id: l.id } },
      upsert: true
    }
  }));
  ...
  return { synced: enriched.length, mode: existingCount > 0 ? 'upsert' : 'insert' };
}
...
async function syncVersionMetadataToMongo({ forceSync } = {}) {
  ...
  for (const landingId of landingIds) {
    ...
    for (const versionId of versionIds) {
      ...
      let content = null;
      let files = null;
      if (metadata.landingType === 'html' && metadata.landingSlug) {
        try {
          const indexPath = path.join(LANDINGS_DIR, metadata.landingSlug, 'index.html');
          if (fs.existsSync(indexPath)) {
            content = fs.readFileSync(indexPath, 'utf-8');
          }
        } catch (e) {
          console.warn(`[store] Could not read HTML content for slug ${metadata.landingSlug}: ${e.message}`);
        }
      } else if (metadata.landingType === 'virtual' && metadata.landingSlug) {
        try {
          const landingDir = path.join(LANDINGS_DIR, metadata.landingSlug);
          if (fs.existsSync(landingDir)) {
            files = [];
            readDirectoryFilesSync(landingDir, '', files);
          }
        } catch (e) {
          console.warn(`[store] Could not read virtual files for slug ${metadata.landingSlug}: ${e.message}`);
        }
      }
  ...
  await col.bulkWrite(ops, { ordered: false });
  return { synced: docs.length, mode: existingCount > 0 ? 'upsert' : 'insert' };
}
...
// Lightweight read — excludes landing content (which can be large HTML blobs)
// Use this for list/dashboard endpoints that only need metadata
async function readDBMeta() {
  ...
  const landings = await mongoDb
    .collection('landings')
    .find({}, { projection: { _id: 0, content: 0, files: 0 } })
    .toArray();
...
// Fetch a single landing's content by id (returns { content, blockData })
async function readLandingContent(id) {
  const engine = getEngine();
  if (engine !== 'mongo') {
    const db = readJsonDB();
    const landing = (db.landings || []).find(l => l.id === id);
    return { content: landing?.content || '', blockData: landing?.blockData || null };
  }

  await connectMongo();
  const landing = await mongoDb
    .collection('landings')
    .findOne({ _id: id }, { projection: { content: 1, blockData: 1, _id: 0 } });
  return { content: landing?.content || '', blockData: landing?.blockData || null };
}

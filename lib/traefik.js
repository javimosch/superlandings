...
async function withTempSshKey(sshKey, callback) {
  if (!sshKey) return callback(null);
...
async function deployTraefikConfig(landing, sshKey = null) {
  ...
  return withTempSshKey(sshKey, async (keyPath) => {
    const commonSshArgs = ['-p', remotePort, '-o', 'StrictHostKeyChecking=no', '-o', 'BatchMode=yes'];
    const commonScpArgs = ['-P', remotePort, '-o', 'StrictHostKeyChecking=no', '-o', 'BatchMode=yes'];
    
    if (keyPath) {
      commonSshArgs.push('-i', keyPath);
      commonScpArgs.push('-i', keyPath);
    }
    ...
    try {
      ...
      throw new Error(`Failed to deploy config via SSH. Ensure SSH authentication is configured correctly for ${remoteHost}`);
    }
  });
}

...
// Internal helper to ensure SSH client is installed (Alpine only)
async function ensureSshClient() {
  try {
    await execCommand('ssh', ['-V']);
  } catch (err) {
    if (err.message.includes('not found') || err.message.includes('ENOENT')) {
      console.log('⚠️ SSH client not found, attempting to install openssh-client via apk...');
      try {
        await execCommand('apk', ['add', '--no-cache', 'openssh-client']);
        console.log('✅ openssh-client installed successfully');
      } catch (apkErr) {
        console.error('❌ Failed to install openssh-client:', apkErr.message);
        throw new Error(`SSH client missing and auto-install failed. Please install openssh-client manually. Error: ${apkErr.message}`);
      }
    } else {
      throw err;
    }
...
async function deployTraefikConfig(landing, sshKey = null) {
  ...
  return withTempSshKey(effectiveSshKey, async (keyPath) => {
    ...
    try {
      console.log(`🔍 Executing: ssh ${commonSshArgs.join(' ')} ${remoteUser}@${remoteHost} "mkdir -p ${remotePath}"`);
      await execCommand('ssh', [...commonSshArgs, `${remoteUser}@${remoteHost}`, `mkdir -p ${remotePath}`]);
      console.log(`✅ Remote directory verified`);

      console.log(`🔍 Executing: scp ${commonScpArgs.join(' ')} ${localConfigPath} ${remoteUser}@${remoteHost}:${remoteFile}`);
      ...
      console.log(`🔍 Executing: ssh ${commonSshArgs.join(' ')} ${remoteUser}@${remoteHost} "ls -la ${remoteFile}"`);
...
async function removeTraefikConfig(landing, sshKey = null) {
  ...
  return withTempSshKey(effectiveSshKey, async (keyPath) => {
    ...
    try {
      console.log(`🔍 Executing: ssh ${commonSshArgs.join(' ')} ${remoteUser}@${remoteHost} "rm -f ${remoteFile}"`);
...
async function testSshConnectivity(sshKey = null) {
  await ensureSshClient();
  const remoteHost = await getTraefikSetting('TRAEFIK_REMOTE_HOST');
  const remoteUser = await getTraefikSetting('TRAEFIK_REMOTE_USER');
  const remotePort = await getTraefikSetting('TRAEFIK_REMOTE_PORT');
  ...
  return withTempSshKey(effectiveSshKey, async (keyPath) => {
    ...
    try {
      console.log(`🔍 Executing: ssh ${commonSshArgs.join(' ')} ${remoteUser}@${remoteHost} "uptime"`);
      const { stdout } = await execCommand('ssh', [...commonSshArgs, `${remoteUser}@${remoteHost}`, 'uptime']);
      return { success: true, output: stdout.trim() };
    } catch (error) {
      const keyHint = getKeyHint(effectiveSshKey);
      throw new Error(`SSH connectivity test failed: ${error.message} (Key hint: ${keyHint})`);
    }
  });
}

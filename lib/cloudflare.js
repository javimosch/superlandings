...
async function getTraefikTargetIp() {
  const ip = await getTraefikSetting('TRAEFIK_REMOTE_HOST');
  if (!ip || !ip.trim()) {
    throw new Error('Missing TRAEFIK_REMOTE_HOST configuration. It is required as DNS target IP.');
  }
  return ip.trim();
}
...
async function desiredRecords(zoneName) {
  const targetIp = await getTraefikTargetIp();
...
function isConflict(existing, desired) {
  ...
  // Only consider A/AAAA/CNAME as conflicts; leave TXT and other records untouched
...
async function configureDnsForDomain(domainInput) {
  ...
  const desired = await desiredRecords(zoneName);
  const targetIp = await getTraefikTargetIp();
  ...
  const info = `DNS configured for ${zoneName}. Root domain (${zoneName}) and any subdomain (*.${zoneName}) will point to ${targetIp}. The www subdomain (www.${zoneName}) aliases the root domain. DNS propagation may take some time.`;

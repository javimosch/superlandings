...
// isValidSlug is shared from lib/utils so all routes use the same guard.
...
async function domainStaticMiddleware(req, res, next) {
  ...
  if (landing) {
    ...
    // Use originalUrl, not url — Express app.use('/*') strips the path from req.url
    let filePath = (req.originalUrl || req.url).split('?')[0];
